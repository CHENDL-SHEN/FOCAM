...
def get_params():
    ...
    parser.add_argument('--expName', default='CAM_train0_PubBUSI', type=str)
    parser.add_argument('--compName', default='CLS', type=str)
...
def main(args):
    ...
    evaluatorA = busi_s1_infer_cam.evaluator(args.dataset, domain=val_domain, SSTB=False, refine_list=[0])
    ...
    train_meter = Average_Meter(['loss', 'cls_loss'])
    ...
    for iteration in range(max_iteration):
        ...
        if args.dataset != 'PubDB':
            if (iteration + 1) % log_iteration == 0:
                ...
                loss_v, cls_loss_v = train_meter.get(clear=True)
        ...
        if (iteration + 1) % val_iteration == 0:
            mIoU, para = evaluatorA.evaluate(model, Q_model=None, ite=args.th, dCRF_iter=0)[0]
